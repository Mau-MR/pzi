...
Point calcGravityPoint(Vec3f result, Point gp) {
	//Get important coordinates
	int xn = result[0] - result[2];
	int yn = result[1] + result[2];

	Point unitarygp((xn - gp.x) / result[2] * 2, (yn - gp.y) / result[2] * 2);
